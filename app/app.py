...
import mit as mit  # Import mit_module and ptb_module if they are Python modules in your project
import ptb as ptb
...
def main():
    ...
    if choice == "Visualize ECG":
        ...
        if selected_context == "Training MIH":
            ...
            uploaded_files = st.sidebar.file_uploader("Select ECG Files (XYZ, HEA, DAT)", type=["xyz", "hea", "dat"], accept_multiple_files=True)
                    ...
                    if modele_selected != 'WaveNet':
                        ...
                        if sampling_option != "None":
                            if sampling_option == "Oversampling":
                                model_path = f'app/models/{modele_selected}/{modele_selected.lower() + "_over_mit"}.h5'
                            elif sampling_option == "Undersampling":

                                model_path = f'app/models/{modele_selected}/{modele_selected.lower() + "_under_mit"}.h5'
                        else:
            ...
            if modele_selected:
                ...
                if st.sidebar.button("Perform the prediction"):
                    ...
                    if modele_selected != 'WaveNet':
                        ...
                        if sampling_option != "None":
                            if sampling_option == "Oversampling":
                                model_path = f'app/models/{modele_selected}/{modele_selected.lower() + "_over_ptb"}.h5'
                            elif sampling_option == "Undersampling":

                                model_path = f'app/models/{modele_selected}/{modele_selected.lower() + "_under_ptb"}.h5'
                        else:
